...
try:
  import numpy as np
  from it01.local import Form, batched, filled, found, prompt, reader, room, schema, sizes, spans, wanted, windows, words
...
class Fake:
  def encode(self, text, add_special_tokens=False):
    ...
    said = tail.split()
    return Coded([9] * head.count("[C]") + [2] + [3] * len(said), ["[C]"] * head.count("[C]") + ["[SEP_TEXT]"] + ["▁" + w for w in said])
  def token_to_id(self, name): return {"[SEP_TEXT]": 2, "[C]": 9}.get(name, 1)
...
class TestLocal(unittest.TestCase):
  ...
  def test_the_schema_names_the_form_and_every_field_twice(self):
    said = schema(Form("statement_of_emoluments", (("salary", "the gross pay"), ("tax_withheld", "tax already taken off"))))
    self.assertIn("[P] statement_of_emoluments", said)
    for name in ("salary", "tax_withheld"):
      self.assertEqual(said.count(name), 2, name)
    self.assertEqual(said.count("[C]"), 2)
  ...
  def test_a_model_file_that_answers_with_other_names_is_refused(self):
    with self.assertRaisesRegex(ValueError, "\\['valid_mask'\\]"):
      spans(Model(gives=GIVES[:-1]), Fake(), words("a b"), FORM, sized())
  ...
  def test_a_model_file_that_answers_in_other_shapes_is_refused(self):
    with self.assertRaisesRegex(ValueError, "indices in 3 dimensions"):
      spans(Model(dims=3), Fake(), words("a b"), FORM, sized())
  ...
  def test_a_tokeniser_that_splits_the_words_differently_is_refused(self):
    with self.assertRaisesRegex(ValueError, "cannot be traced"): prompt(Halved(), words("a b c"), FORM)

  def test_a_tokeniser_that_does_not_know_the_marks_is_refused(self):
    with self.assertRaisesRegex(ValueError, "no \\[SEP_TEXT\\]"): prompt(Nameless(), words("a b"), FORM)

  def test_a_tokeniser_that_marks_no_line_is_refused(self):
    with self.assertRaisesRegex(ValueError, "marked 0 of 1 lines"): prompt(Deaf(), words("a b"), FORM)

  def test_a_span_names_the_words_it_covers(self):
    self.assertEqual(spans(Model(), Fake(), words("pay 1,200.00"), FORM, sized()), {"salary": [(100, 1, 6)]})

  def test_a_span_below_the_confidence_is_left_out(self):
    self.assertEqual(spans(Model(logit=-1.0), Fake(), words("pay 1,200.00"), FORM, sized()), {"salary": []})

  def test_a_span_reaching_past_the_document_is_left_out(self):
    self.assertEqual(spans(Model(last=9), Fake(), words("a b"), FORM, sized()), {"salary": []})

  def test_a_confidence_below_what_a_number_holds_is_still_read(self):
    self.assertEqual(spans(Model(logit=-1e4), Fake(), words("pay 1,200.00"), FORM, sized()), {"salary": []})

  def test_a_document_is_read_into_facts(self):
    with mock.patch("it01.local.reader", lambda: (Model(), Fake())), mock.patch("it01.local.data", lambda name: HELD):
  ...
  def test_a_span_that_holds_no_figure_is_left_out(self):
    with mock.patch("it01.local.reader", lambda: (Model(last=2), Fake())), mock.patch("it01.local.data", lambda name: HELD):
      self.assertEqual(found("pay emoluments"), ())

  def test_a_document_that_fits_is_read_in_one_window(self):
    self.assertEqual(windows(Fake(), words("a b c"), FORM, {"input_ids": 64, "tw_idx": 64}), [(0, 3)])

  def test_a_document_too_long_is_read_in_overlapping_windows(self):
    self.assertEqual(windows(Fake(), words("a b c d e f"), FORM, {"input_ids": 6, "tw_idx": 64}), [(0, 3), (2, 3), (4, 2)])

  def test_a_window_never_holds_more_words_than_the_model_takes(self):
    self.assertEqual(windows(Fake(), words("a b c d e f"), FORM, {"input_ids": 64, "tw_idx": 2}),
                     [(0, 2), (1, 2), (2, 2), (3, 2), (4, 2)])

  def test_a_form_leaving_no_room_is_refused(self):
    with self.assertRaisesRegex(ValueError, "leaves no room for the document"): room(Fake(), words("a b"), FORM, 2)
  ...
  def test_the_same_words_seen_from_two_windows_are_reported_once(self):
    with mock.patch("it01.local.reader", lambda: (Model(cap=15, each=[(10, 11), (1, 2)]), Fake())), \
         mock.patch("it01.local.data", lambda name: HELD):
  ...
  def test_a_form_with_no_name_is_refused(self):
    with mock.patch("it01.local.data", lambda name: {"form": {"fields": {"salary": "pay"}}}):
      with self.assertRaisesRegex(ValueError, "a form with a name"): wanted()
