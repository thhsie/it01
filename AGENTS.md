...
6. **One dependency per boundary.** Computation imports the standard library and itself, nothing else, so working out a tax result needs no install beyond the package. Exactly one module, `it01/local.py`, may import a model runtime, and only under the `local` extra. Exactly one module, `it01/paper.py`, may import a reader of a document format, and only under the `pdf` extra. Everything else is written, not installed. `test_privacy.py` names each module and the imports it may have.
